...
const banner = svgFrame({
  ...
  height: 520,
  body: `
  <rect x="72" y="72" width="1456" height="376" rx="36" fill="#ffffff" stroke="${colors.line}" stroke-width="3"/>
  <g transform="translate(126 120) scale(0.58)">${logo.match(/<rect[\s\S]*<\/svg>/)[0].replace("</svg>", "")}</g>
  <text x="430" y="175" font-family="Inter, -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans CJK SC', sans-serif" font-size="58" font-weight="800" fill="${colors.ink}">Skill Toolkit 公开包</text>
  <text x="430" y="235" font-family="Inter, -apple-system, BlinkMacSystemFont, 'PingFang SC', sans-serif" font-size="30" fill="${colors.muted}">把重复工作整理成 Codex / Claude Code 可调用的 Agent Skill</text>
  <g font-family="Inter, -apple-system, BlinkMacSystemFont, sans-serif" font-size="24" font-weight="700">
    <rect x="430" y="290" width="172" height="52" rx="16" fill="#e7f5ff"/><text x="458" y="324" fill="${colors.blue}">5 Skills</text>
    <rect x="626" y="290" width="170" height="52" rx="16" fill="#ebfbee"/><text x="654" y="324" fill="${colors.green}">Codex</text>
    <rect x="820" y="290" width="236" height="52" rx="16" fill="#fff3bf"/><text x="848" y="324" fill="#8a5a00">Claude Code</text>
    <rect x="1080" y="290" width="212" height="52" rx="16" fill="#f1f3f5"/><text x="1108" y="324" fill="${colors.ink}">中文主入口</text>
  </g>
  `,
});
...
const social = svgFrame({
  ...
  height: 640,
  body: `
  <rect x="54" y="54" width="1172" height="532" rx="44" fill="#ffffff" stroke="${colors.line}" stroke-width="3"/>
  <text x="104" y="150" font-family="Inter, -apple-system, BlinkMacSystemFont, 'PingFang SC', sans-serif" font-size="54" font-weight="800" fill="${colors.ink}">Skill Toolkit Public Pack</text>
  <text x="104" y="214" font-family="Inter, -apple-system, BlinkMacSystemFont, 'PingFang SC', sans-serif" font-size="30" fill="${colors.muted}">创建、采用、叠加、修复和演化 Agent Skills</text>
  <g transform="translate(104 280)">
    ${["Adopt", "Overlay", "Create", "Modify", "Evolve"].map((label, i) => `<rect x="${i * 210}" y="0" width="172" height="74" rx="20" fill="${["#e7f5ff", "#ebfbee", "#fff3bf", "#fff0f6", "#f1f3f5"][i]}" stroke="${colors.line}"/><text x="${i * 210 + 34}" y="48" font-family="Inter, sans-serif" font-size="26" font-weight="800" fill="${colors.ink}">${label}</text>`).join("")}
  </g>
  <text x="104" y="500" font-family="Inter, -apple-system, BlinkMacSystemFont, sans-serif" font-size="25" fill="${colors.muted}">kun-agent-system/skill-toolkit-public</text>
  `,
});
